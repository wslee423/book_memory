    ...
    <main className="min-h-screen bg-white px-4 py-8 max-w-7xl mx-auto">
      <div className="animate-pulse">
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-gray-100 rounded-lg px-4 py-3 w-20 h-16" />
          ))}
        </div>
        {/* 검색창 */}
        <div className="h-9 w-80 bg-gray-100 rounded mb-4" />
        {/* 필터 */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-2">
              <div className="aspect-[5/7] bg-gray-100 rounded" />
