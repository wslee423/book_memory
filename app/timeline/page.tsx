...
export default async function TimelinePage() {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-gray-950 px-4 py-8 max-w-3xl mx-auto">
      <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-8">독서 회고</h1>

      {timeline.length === 0 ? (
        <p className="text-gray-400 dark:text-gray-500 text-sm">완독 기록이 없습니다.</p>
      ) : (
                  ...
                  {books.map((book) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100 leading-tight line-clamp-2">{book.title}</p>
                        {book.author && <p className="text-xs text-gray-500 dark:text-gray-400">{book.author}</p>}
