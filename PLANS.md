...
### 작업 목록
- [x] merge-pages 실행 — 분할된 메모 병합 (999개 → 141개)
- [x] re-embed — 전체 재임베딩 (54건 성공, 이미지 2건 URL 만료 제외)
- [x] TD-005: `/bookshelf` loading.tsx 생성
- [x] TD-008: BookGallery `next/image` sizes 최적화
- [x] TD-010: BookPageItem `<img>` → `next/image`
- [x] TD-015: `BookStatus` 유니온 타입 도입
- [ ] 폴리싱 작업 (진행 중 — 운섭 요청 기반)
- [ ] 다크모드
